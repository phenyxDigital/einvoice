...
use DateTime;
use PhenyxInvoicing\AllowanceOrCharge;
use PhenyxInvoicing\Attachment;
use PhenyxInvoicing\Delivery;
use PhenyxInvoicing\Identifier;
use PhenyxInvoicing\Invoice;
use PhenyxInvoicing\InvoiceLine;
use PhenyxInvoicing\Models\InvoiceTotals;
use PhenyxInvoicing\Party;
use PhenyxInvoicing\Payments\Card;
use PhenyxInvoicing\Payments\Mandate;
use PhenyxInvoicing\Payments\Payment;
use PhenyxInvoicing\Payments\Transfer;
use EASYXML\EASYXML;
...
class UblWriter extends AbstractWriter {
    ...
    public function export(Invoice $invoice): string{
        ...
        $xml = EASYXML::newInstance($rootElementName, null, [
    ...
    /**
     * Add identifier node
     * @param EASYXML       $parent     Parent element
     ...
     */
    private function addIdentifierNode(EASYXML $parent, string $name, Identifier $identifier, string $schemeAttr = "schemeID") {
     ...
     */
    private function addPeriodNode(EASYXML $parent, $source) {
     ...
     */
    private function addOrderReferenceNode(EASYXML $parent, Invoice $invoice) {
     ...
     * @param Invoice $invoice Invoice instance
     */
    private function addTenderOrLotReferenceNode(EASYXML $parent, Invoice $invoice) {
     ...
     */
    private function addAmountNode(EASYXML $parent, string $name, float $amount, string $currency) {
    ...
    private function addVatNode(
        EASYXML $parent, string $name, string $category,  ? float $rate,
     ...
     * @return EASYXML                   Postal address node
     */
    private function addPostalAddressNode(EASYXML $parent, string $name, $source) {
     ...
     * @param EASYXML  $parent Invoice element
     * @param Party $party  Party instance
     */
    private function addSellerOrBuyerNode(EASYXML $parent, Party $party) {
     ...
     * @param Party $party  Party instance
     */
    private function addPayeeNode(EASYXML $parent, Party $party) {
     ...
     */
    private function addDeliveryNode(EASYXML $parent, Delivery $delivery) {
     ...
     */
    private function addPaymentNodes(EASYXML $parent, Payment $payment,  ? DateTime $dueDate) {
     ...
     * @param EASYXML $parent PaymentMeans element
     * @param Card $card   Card instance
     */
    private function addPaymentCardNode(EASYXML $parent, Card $card) {
     ...
     */
    private function addPaymentTransferNode(EASYXML $parent, Transfer $transfer) {
     ...
     */
    private function addPaymentMandateNode(EASYXML $parent, Mandate $mandate) {
     ...
     */
    private function addTaxTotalNodes(EASYXML $parent, InvoiceTotals $totals) {
     ...
     * @param InvoiceTotals $totals Invoice totals
     */
    private function addDocumentTotalsNode(EASYXML $parent, InvoiceTotals $totals) {
    ...
    /**
     * Add invoice line
     * @param EASYXML        $parent              Parent XML element
     ...
     */
    private function addAttachmentNode(EASYXML $parent, Attachment $attachment) {
