...
use PhenyxInvoicing\Writers\UblWriter;
use EASYXML\EASYXML;
...
class UblReader extends AbstractReader {
    ...
    public function import(string $document): Invoice{
        ...
        // Load XML document
        $xml = EASYXML::fromString($document);
     ...
     * @param  EASYXML       $xml        XML node
     * @param  string     $schemeAttr Scheme attribute name
     * @return Identifier             Identifier instance
     */
    private function parseIdentifierNode(EASYXML $xml, string $schemeAttr = "schemeID"): Identifier{
     ...
     */
    private function parsePeriodFields(EASYXML $xml, $target) {
     ...
     */
    private function parsePostalAddressFields(EASYXML $xml, $target) {
     ...
     */
    private function parseSellerOrBuyerNode(EASYXML $xml): Party{
     ...
     * @return Party      Party instance
     */
    private function parsePayeeNode(EASYXML $xml): Party{
     ...
     */
    private function parseDeliveryNode(EASYXML $xml): Delivery{
     ...
     */
    private function parsePaymentNodes(EASYXML $xml):  ? Payment{
        ...
        if ($meansNode === null && $termsNode === null) {
            return null;
        }
     ...
     * @param  EASYXML $xml Payment card node
     * @return Card      Card instance
     */
    private function parsePaymentCardNode(EASYXML $xml) : Card{
     ...
     * @param  EASYXML     $xml Payment transfer node
     * @return Transfer      Transfer instance
     */
    private function parsePaymentTransferNode(EASYXML $xml): Transfer{
     ...
     * @param  EASYXML    $xml Payment mandate node
     * @return Mandate      Mandate instance
     */
    private function parsePaymentMandateNode(EASYXML $xml): Mandate{
     ...
     */
    private function setVatAttributes($target, EASYXML $xml, array $taxExemptions) {
     ...
     * @param array               &$taxExemptions Tax exemption reasons
     */
    private function addAllowanceOrCharge($target, EASYXML $xml, array &$taxExemptions) {
     ...
     */
    private function parseInvoiceLine(EASYXML $xml, array &$taxExemptions): InvoiceLine{
     ...
     */
    private function parseAttachmentNode(EASYXML $xml): Attachment{
